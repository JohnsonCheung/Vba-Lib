...
With O
    For J = 0 To UB(Ly)
        LinAsgTTRst A_Ly(J), A, B, C
        Select Case A
        Case C1_Lo
            Select Case B
                Case "Hid": LVF_Push3 .HidB, J, C2_Lo_Hid, C
                Case "Fld": LVF_Push3 .FnyB, J, C2_Lo_Fld, C
                Case "Nm":  LVF_Push3 .NmB, J, C2_Lo_Nm, C
            End Select
        Case C1_Bdr
            Select Case B
                Case C2_Bdr_L: LVF_Push3 .BdrB.Left, J, C2_Bdr_L, C
                Case C2_Bdr_R: LVF_Push3 .BdrB.Right, J, C2_Bdr_R, C
                Case C2_Bdr_C: LVF_Push3 .BdrB.Column, J, C2_Bdr_C, C
            Case Else: Push .ErLy, FmtQQ("Lx(?) T2(?) should be [Left Right Col]", J, B)
            End Select
        Case C1_Tot
            Select Case B
                Case C2_Tot_Sum: LVF_Push3 .TotB.Sum, J, C2_Tot_Sum, C
                Case C2_Tot_Avg: LVF_Push3 .TotB.Avg, J, C2_Tot_Avg, C
                Case C2_Tot_Cnt: LVF_Push3 .TotB.Cnt, J, C2_Tot_Cnt, C
            Case Else:  Push .ErLy, FmtQQ("Lx(?) T2(?) should be [Tot Avg Cnt]", J, B)
            End Select
            Case C1_Fmt: LVF_Push3 .FmtB, J, B, C
            Case C1_Wdt: LVF_Push3 .WdtB, J, B, C
            Case C1_Lvl: LVF_Push3 .LvlB, J, B, C
            Case C1_Cor: LVF_Push3 .CorB, J, B, C
            Case C1_Tit: LFV_Push3 .TitB, J, B, C
            Case C1_Lbl: LFV_Push3 .LblB, J, B, C
            Case C1_Fml: LFV_Push3 .FmlB, J, B, C
            Case C1_Bet: LFV_Push3 .BetB, J, B, C
        Case Else
...
End Function
Private Function LCFVAy_FldAy(A() As P3LCFV) As String()
Dim O$(), J%
For J = 0 To UB(A)
    Push O, A(J).Fld
Next
