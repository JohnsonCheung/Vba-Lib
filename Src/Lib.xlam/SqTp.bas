...
Private Function SwBrk_BoolOpt(A As SwBrk, Pm As Dictionary, Sw As Dictionary) As BoolOpt
If Sw.Exists(A.Nm) Then Exit Function
Dim Ay$(): Ay = A.TermAy
Dim Z As BoolOpt
Select Case A.OpStr
Case "OR":  Z = SwTermAy_BoolOpt(Ay, "OR", Pm, Sw)
Case "AND": Z = SwTermAy_BoolOpt(Ay, "AND", Pm, Sw)
Case "NE":  Z = SwT1T2_BoolOpt(Ay(0), Ay(1), "NE", Pm, Sw)
Case "EQ":  Z = SwT1T2_BoolOpt(Ay(0), Ay(1), "EQ", Pm, Sw)
Case Else: Stop
End Select
If Not Z.Som Then Exit Function
SwBrk_BoolOpt = SomBool(Z.Bool)
End Function
...
Private Function SwT1T2_BoolOpt(T1$, T2$, EqNeOpStr$, Pm As Dictionary, Sw As Dictionary) As BoolOpt
Dim S1$, S2$
