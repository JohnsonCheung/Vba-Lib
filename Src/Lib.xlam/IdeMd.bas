...
If ValIsLngAy(A) Then DftCmpTyAy = A
End Function
...
End Function
Function PjMd(A As VBProject, MdNm) As CodeModule
...
Dim Pj As VBProject
Set Pj = MdPj(A)
If PjHasMdNm(A, ToMdNm) Then
    Er "MdCpy", "{Pj} already contains {ToMdNm}.  {Md} cannot be copied", PjNm(Pj), ToMdNm, MdNm(A)
End If
...
Function PjEnsMd(A As VBProject, MdNm$, Optional Ty As vbext_ComponentType = vbext_ct_StdModule) As CodeModule
If Not PjHasMd(A, MdNm) Then
    Set PjEnsMd = PjCrtMd(A, MdNm, Ty)
Else
    Set PjEnsMd = PjMd(A, MdNm)
End If
End Function
...
If LnoCnt_Sz(LnoCntAy) = 0 Then Exit Sub
Dim J%, M&
M = LnoCntAy(0).Lno
For J = 1 To LnoCnt_UB(LnoCntAy)
    If M > LnoCntAy(J).Lno Then Stop
    M = LnoCntAy(J).Lno
Next

For J = LnoCnt_UB(LnoCntAy) To 0 Step -1
...
Sub MdReportSorting(A As CodeModule, Optional Opt As eReportSortingOption = eBothDifAndSam)
