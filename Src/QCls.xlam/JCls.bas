...
Property Get MayLCC(Som As Boolean, Optional LCC As LCC) As MayLCC
Dim O As New MayLCC
Set MayLCC = O.Init(Som, LCC)
End Property
...
Property Get NonLCC() As MayLCC
Static X As Boolean, Y As MayLCC
If Not X Then
    X = True
    Set Y = MayLCC(False)
End If
...
Property Get SomLCC(LCC As LCC) As MayLCC
Set SomLCC = MayLCC(True, LCC)
End Property

Property Get FmCnt(FmLno, Cnt) As FmCnt
Dim O As New FmCnt
