...
Function SyAddAp(ParamArray Str_or_Sy_Ap()) As String()
...
Function AyAsg(Ay, OIntoAy)
If TypeName(Ay) = TypeName(OIntoAy) Then
    AyAsg = Ay
    Exit Function
End If
If AyIsEmp(Ay) Then
...
Sub AyAsgEle(Ay, ParamArray OAp())
Dim V, J%
For Each V In Ay
...
AyBytAy = AyAsg(Ay, Emp.BytAy)
End Property

Property Get AyDblAy(Ay) As Double()
AyDblAy = AyAsg(Ay, Emp.DblAy)
End Property
...
Dim O1$()
    ...
    Next
Stop
'If IsEmp(O1) Then Exit Property
Dim O2$()
...
If AyIsEmp(Ay) Then AyRmvEmp = Ay: Exit Property
Dim O: O = Ay: Erase O
Dim I
For Each I In Ay
...
Dim O
    O = Ay
    If Not FmTo.IsEmp Or AyIsEmp(Ay) Then
        Dim FmI&, ToI&
        FmI = FmTo.FmIx
        ToI = FmTo.ToIx
        Dim I&, J&, U&
