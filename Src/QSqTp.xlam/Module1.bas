...
For Each K In A.Keys
    ZPushObj O, S1S2(K & vbCrLf & ZLines_UnderLin(K) & vbCrLf & A(K), "!" & "Er AExcess")
Next
...
Dim O() As S1S2, K
For Each K In A.Keys
    ZPushObj O, S1S2("!" & "Er BExcess", K & vbCrLf & ZLines_UnderLin(K) & vbCrLf & A(K))
Next
...
Dim O() As S1S2, K
For Each K In A.Keys
    ZPushObj O, S1S2("*Same", K & vbCrLf & ZLines_UnderLin(K) & vbCrLf & A(K))
Next
ZDCRslt_Ly__Sam = O
End Property
...
Property Get ZDCRslt_Ly(A As DCRslt) As String()
With A
...
ZIs_NmChr = False
End Property
Property Get ZMd_Dic(A As CodeModule) As Dictionary
Set ZMd_Dic = ZSrc_Dic(ZMd_Src(A), ZMd_PjNm(A), ZMd_Nm(A))
End Property
Sub ZZ_ZMd_Cmp()
ZMd_Cmp ZMd("QTool.F_Tool"), ZMd("QSqTp.F_Tool")
End Sub
Sub ZMd_Cmp(A As CodeModule, B As CodeModule)
Dim A1 As Dictionary, B1 As Dictionary
    Set A1 = ZMd_Dic(A)
    Set B1 = ZMd_Dic(B)
Dim C As DCRslt
    C = ZDic_Cmp(A1, B1, ZMd_DNm(A), ZMd_DNm(B))
AyBrw ZDCRslt_Ly(C)
End Sub
Property Get ZIs_Nothing(A) As Boolean
ZIs_Nothing = TypeName(A) = "Nothing"
End Property
Property Get ZIs_MdNm(A) As Boolean
...
Property Get ZMd(MdDNm) As CodeModule
Dim A$: A = MdDNm
Dim P As VBProject
Dim MdNm$
    Dim L%
...
Set ZMd_MthDrs = Drs(ZSplitSsl(""), ZMd_MthDry(A))
End Property
...
Property Get ZMd_MthNy(A As CodeModule, Optional MthNmPatn$ = ".", Optional IsNoMdNmPfx As Boolean, Optional Mdy0$) As String()
Dim Ay$(): Ay = ZSrc_MthNy(ZMd_Src(A), MthNmPatn, Mdy0:=Mdy0)
If IsNoMdNmPfx Then
    ZMd_MthNy = Ay
Else
...
End If
End Property
Property Get ZDft_Pj(PjNm0$)
If PjNm0 = "" Then
    Set ZDft_Pj = ZCurPj
Else
    Set ZDft_Pj = ZPj(PjNm0)
End If
End Property
Property Get ZDft_Md(MdDNm0$)
If MdDNm0 = "" Then
    Set ZDft_Md = ZCurMd
Else
    Set ZDft_Md = ZMd(MdDNm0)
End If
End Property
...
Dim Ny$(), O$(), I
Ny = ZMd_MthNy(A, Mdy0:="Public", IsNoMdNmPfx:=True)
ZPushAyNoDup Ny, ZMd_MthNy(A, "ZZ_", IsNoMdNmPfx:=True)
...
Property Get ZMdy_IsSel(A$, MdyAy$()) As Boolean
If ZSz(MdyAy) = 0 Then ZMdy_IsSel = True: Exit Property
Dim Mdy
For Each Mdy In MdyAy
    If Mdy = "Public" Then
        If A = "" Then ZMdy_IsSel = True: Exit Property
    End If
    If A = Mdy Then ZMdy_IsSel = True: Exit Property
Next
End Property
...
Sub ZMth_Cpy_ToMd(A As Mth, ToMd As CodeModule, Optional IsSilent As Boolean)
...
If Not IsSilent Then
    Debug.Print FmtQQ("ZMth_Cpy_ToMd: Mth(?) is copied ToMd(?)", ZMth_DNm(A), ZMd_DNm(ToMd))
End If
End Sub
...
End Sub
Property Get ZDft_Mth(MthDNm0$) As Mth
If MthDNm0 = "" Then
    Set ZDft_Mth = ZCurMth
    Exit Property
End If
Set ZDft_Mth = ZMthDNm_Mth(MthDNm0)
End Property
...
Sub ZMth_Rmv(A As Mth, Optional IsSilent As Boolean)
...
Next
If Not IsSilent Then
    Debug.Print FmtQQ("ZMth_Rmv: Mth(?) of LinCnt(?) is deleted", ZMth_DNm(A), Cnt)
End If
End Sub
...
End Sub
Sub ZMd_Rpl_Cxt(A As CodeModule, Cxt$)
Dim N%: N = A.CountOfLines
ZMd_Clr A, IsSilent:=True
A.AddFromString Cxt
Debug.Print ZFmtQQ("ZMd_Rpl_Cxt: Md(?) of Ty(?) of Old-LinCxt(?) is replaced by New-Len(?) New-LinCnt(?).<-----------------", _
    ZMd_DNm(A), ZMd_TyNm(A), N, Len(Cxt), ZLines_LinCnt(Cxt))
End Sub
Property Get ZMd_TyNm$(A As CodeModule)
ZMd_TyNm = ZCmpTy_Nm(ZMd_CmpTy(A))
End Property
Property Get ZStr_SubStrCnt&(A$, SubStr$)
Dim P&, O%, L%
L = Len(SubStr)
P = 1
While P > 0
...
ZStr_SubStrCnt = O
End Property
Property Get ZLines_LinCnt%(A$)
ZLines_LinCnt = ZStr_SubStrCnt(A, vbCrLf) + 1
End Property
...
Property Get ZPj_MthNy(A As VBProject, Optional MthNmPatn$ = ".", Optional MbrNmPatn$ = ".", Optional Mdy0$) As String()
Dim Ay() As CodeModule: Ay = ZPj_MbrAy(A, MbrNmPatn)
If ZSz(Ay) = 0 Then Exit Property
Dim I, O$()
For Each I In Ay
    ZPushAy O, ZMd_MthNy(ZCvMd(I), MthNmPatn, Mdy0:=Mdy0)
Next
O = AyAddPfx(O, A.Name & ".")
ZPj_MthNy = O
End Property
...
If ZSz(Ay) = 0 Then Exit Property
For Each I In Ay
    Dim Pfx$: Pfx = Left(ZMd_Nm(ZCvMd(I)), 2)
...
End Property
Property Get ZMdy0_MdyAy(A$) As String()

End Property
Property Get ZSrc_MthNy(A$(), Optional MthNmPatn$ = ".", Optional IsNoSrt As Boolean, Optional Mdy0$) As String()
Dim L%(): L = ZSrc_AllMthFmLnoAy(A)
If ZSz(L) = 0 Then Exit Property
Dim O$()
    Dim MdyAy$(): MdyAy = ZMdy0_MdyAy(Mdy0)
...
Set ZVbe_DupFFunDrs = Drs(Fny, Dry)
End Property
...
Exit Sub
XX:
    A(I) = S1S2(ZRpl_VBar(A1), ZRpl_VBar(A2))
    Return
End Sub
...
ZAsc_IsUCase = True
End Property
Function ZRmv_Pfx$(A, Pfx$)
If ZIs_Pfx(A, Pfx) Then
    ZRmv_Pfx = Mid(A, Len(Pfx) + 1)
Else
    ZRmv_Pfx = A
End If
End Function
...
Dim Ny$(): Ny = ZMd_MthNy(A, IsNoMdNmPfx:=True, Mdy0:="Public")
